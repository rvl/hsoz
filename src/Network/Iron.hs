...
data Password = Password
  ...
  , encPassword :: SecureMem -- ^ Encryption password
  , intPassword :: SecureMem -- ^ Integrity password
  }
...
password' k e i = Password k (toSecureMem e) (toSecureMem i)
...
generateKey :: Byteable p => Int -> Int -> p -> ByteString -> ByteString
generateKey iterations size p salt = PBKDF2.generate prf params (toBytes p) salt
  where
...
unseal' opts now p cookie = do
  ...
  Password _ enc int <- getPassword opts (ckPasswordId eck) p
  ok <- verify ck int
  case decrypt eck enc of
    Just ctext -> JSON.eitherDecode' (BL.fromStrict ctext)
    Nothing    -> Left "Iron decryption failed"
  where
    decrypt :: EncCookie -> SecureMem -> Maybe ByteString
    decrypt EncCookie{..} sec = text
      where
        EncryptionOpts{..} = ironEncryption opts
        encKey = generateKey ieIterations size sec ckEncSalt
        text = ironDecrypt ieAlgorithm encKey ckIv ckText -- fixme: handle fail
        size = keySize ieAlgorithm

    verify :: Cookie -> SecureMem -> Either String ()
    verify Cookie{..} sec = if constEqBytes ckIntDigest macDigest
                            then Right ()
                            else Left "Bad hmac value"
      where
        IntegrityOpts{..} = ironIntegrity opts
        macKey = generateKey iiIterations size sec ckIntSalt
